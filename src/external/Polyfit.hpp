...
{
    template<typename T>
    std::vector<T> polyfit(const std::vector<T>& xValues, const std::vector<T>& yValues, const int degree, const std::vector<T>& weights = std::vector<T>(), bool useJacobi = true)
    {
    ...
    T polyval(const std::vector<T>& coeffs, const T x)
    {
        T   result = 0;
        int alpha = 0;
        for (T coeff : coeffs) { result = result + coeff * std::pow(x, alpha);  alpha++; }
        return result;
    }
